...
export default function FolderPanel({
  ...
  return (
    <>
      {isMobileOpen && (
        <div className="lg:hidden fixed inset-0 bg-black bg-opacity-50 z-40" onClick={onMobileToggle} />
      )}

      <aside
        className={`
          fixed lg:sticky top-0 left-0 h-screen
          w-64 bg-white flex flex-col z-50
        ...
        <div className="flex-1 overflow-y-auto bg-white">
          <div className="px-2">
            <FolderList
              ...
              onFolderClick={(folder) => { onFolderClick(folder); onMobileToggle() }}
              onRename={onRename}
              onDelete={onDelete}
            />
          </div>
